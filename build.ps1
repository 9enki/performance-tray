# PerformanceTray をリリースビルドして bin\PerformanceTray.exe に置く。
...
# 起動中の PerformanceTray があると exe を上書きできないので、先に終了するか install.ps1 を使う。
...
Copy-Item (Join-Path $env:CARGO_TARGET_DIR 'release\PerformanceTray.exe') bin\PerformanceTray.exe -Force
"PerformanceTray $ver -> " + (Get-Item bin\PerformanceTray.exe).FullName + " (" + (Get-Item bin\PerformanceTray.exe).Length + " bytes)"
