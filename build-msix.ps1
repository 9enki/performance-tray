...
#   .\build-msix.ps1                                   # 仮の Identity で dist\PerformanceTray_<ver>.msix を作る（構成確認用）
#   .\build-msix.ps1 -IdentityName 12345Genki.PerformanceTray -Publisher 'CN=xxxxxxxx-...' -PublisherDisplayName Genki
# Identity の各値は Partner Center でアプリ名を予約すると発行される。署名は Store が行うので、ここでは署名しない。
param(
    [string]$IdentityName = 'PLACEHOLDER.PerformanceTray',
...
if (-not (Test-Path .\bin\PerformanceTray.exe)) { throw 'bin\PerformanceTray.exe がありません' }
...
Copy-Item .\bin\PerformanceTray.exe $stage
...
$out = Join-Path $OutDir "PerformanceTray_$ver.msix"
