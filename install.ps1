<#
.SYNOPSIS
  PerformanceTray を自分用にインストール・更新・アンインストールする。
.DESCRIPTION
  ビルド → 起動中のインスタンスを停止 → %LOCALAPPDATA%\Programs\PerformanceTray\ に上書きコピー → 起動。
  スタート メニューにもショートカットを置くので、スタートの検索（Windows キー → performance）から起動できる。
  bin\ など別の場所から起動したときに残る古いトレイアイコンの設定も消し、
  「その他のシステム トレイ アイコン」の一覧にはインストール先の分だけが並ぶようにする。
...
.PARAMETER Show
  起動時に渡す --show の値（cpu / memory / both）。省略時は今の自動起動設定の値、それも無ければ既定の both。
.PARAMETER Startup
  ログイン時に自動起動する。
.PARAMETER NoStartup
  ログイン時に自動起動しない。
.PARAMETER NoBuild
  ビルドを省略し、既存の bin\PerformanceTray.exe をそのまま配置する。
.PARAMETER NoLaunch
  配置後に起動しない。
.PARAMETER Uninstall
  アプリを停止し、インストール先フォルダー・スタート メニューのショートカット・自動起動の設定・
  トレイアイコンの表示設定を削除する。
.EXAMPLE
  .\install.ps1                        # ビルドして入れ替え、起動（更新もこれ 1 回）
  .\install.ps1 -Startup               # 自動起動を有効にして入れ替え
  .\install.ps1 -NoStartup             # 自動起動を無効にして入れ替え
  .\install.ps1 -Show cpu -Startup     # CPU だけ表示にして自動起動を有効化
...
$installDir = Join-Path $env:LOCALAPPDATA 'Programs\PerformanceTray'
$target = Join-Path $installDir 'PerformanceTray.exe'
# スタートの検索は Start Menu\Programs 配下のショートカットを見るので、exe を置くだけでは候補に出ない
$shortcut = Join-Path ([Environment]::GetFolderPath('Programs')) 'PerformanceTray.lnk'
# タスクバー設定「その他のシステム トレイ アイコン」の元データ。exe のパスごとに 1 アイコン 1 キー
$notifyKey = 'HKCU:\Control Panel\NotifyIconSettings'
$runKey = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'
$approvedKey = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run'
$valueName = 'PerformanceTray'

function Stop-App {
    $procs = @(Get-Process PerformanceTray -ErrorAction SilentlyContinue)
    if ($procs.Count -gt 0) {
        $procs | Stop-Process -Force
        Start-Sleep -Milliseconds 800
        "停止: PerformanceTray ($($procs.Count) プロセス)"
    }
}
...
function Get-StartupArgs {
    # "C:\...\PerformanceTray.exe" --show X から引数部分だけを取り出す
...
function Set-StartMenuShortcut {
    ...
    $link.Arguments = $AppArgs
    $link.WorkingDirectory = $installDir
    $link.IconLocation = "$target,0"
    $link.Description = 'CPU とメモリの使用率をタスクトレイに数字で表示する'
    $link.Save()
    "ショートカット: $shortcut"
}
...
function Remove-TrayIconSettings {
    # Windows はトレイアイコンの表示設定を exe のパス単位で保存するので、bin\ や別の場所から起動した分が
    # 「その他のシステム トレイ アイコン」に古い行として残る。インストール先以外のパスの登録を消す。
    # -IncludeTarget を付けるとインストール先の分も消す（アンインストール用）
    ...
    Get-ChildItem $notifyKey -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        if (-not $path) { return }
        if ((Split-Path $path -Leaf) -ne 'PerformanceTray.exe') { return }
        if (-not $IncludeTarget -and $path -eq $target) { return }
...
if ($Uninstall) {
    ...
    Remove-TrayIconSettings -IncludeTarget
...
if (-not $NoBuild) {
    Stop-App | Out-Null   # 起動中だと bin\PerformanceTray.exe を上書きできない
    & .\build.ps1
    if ($LASTEXITCODE -ne 0) { throw 'ビルドに失敗しました' }
}
if (-not (Test-Path .\bin\PerformanceTray.exe)) { throw 'bin\PerformanceTray.exe がありません。先に build.ps1 を実行してください' }

$firstInstall = -not (Test-Path $target)
$startupWasOn = [bool](Get-StartupCommand)
# 起動引数。-Show 指定 > 今の自動起動設定の値 > なし（既定 both）
$appArgs = if ($Show -and $Show -ne 'both') { "--show $Show" } elseif ($Show -eq 'both') { '' } else { Get-StartupArgs }

Stop-App
New-Item -ItemType Directory -Force -Path $installDir | Out-Null
Copy-Item .\bin\PerformanceTray.exe $target -Force
"配置: $target"
Set-StartMenuShortcut -AppArgs $appArgs
Remove-TrayIconSettings
...
if (-not $NoLaunch) {
    ...
    if (Get-Process PerformanceTray -ErrorAction SilentlyContinue) { "起動: $target $appArgs" } else { Write-Warning '起動を確認できませんでした' }
}

if ($firstInstall) {
    ''
    '初回はタスクバー右端の「^」の中に入ります。常時表示にするには'
    '  設定 > 個人用設定 > タスクバー > その他のシステム トレイ アイコン で PerformanceTray をオンにしてください。'
